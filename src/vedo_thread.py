...
class VedoThread(Plotter):
    ...
    def __init__(self,
                 ...
                 main_fnc: Optional[Callable] = None):
        """
        Vedo-based visualizer for SOFA scenes.

        :param root_node: Root node of the SOFA scene graph.
        :param main_fnc: Timer callback function to execute. Default is a call to Sofa.Simulation.animate().
        """
    ...
    def show(self, *args, **kwargs):
        ...
        t = Thread(target=self.thread, daemon=False)
        t.start()
        Plotter.show(self, *args, **kwargs).close()
        t.join()
