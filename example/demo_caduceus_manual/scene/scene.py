...
class Scene(Sofa.Core.Controller):
    ...
    def create_graph(self):
        ...
        self.root.snake.addObject('MechanicalObject', name='SnakeMO', src='@Grid', scale=1, dy=2)
    ...
    def init_viewer(self, viewer: vedo.Plotter):

        vedo_objects = []

        # Create the snake
        snake_ogl = self.root.snake.visual.body.getObject('OglBody')
        self.mesh_snake = vedo.Mesh([snake_ogl.position.value, snake_ogl.quads.value])
        vedo_objects.append(self.mesh_snake)

        # Add all objects
        viewer.add(*vedo_objects)
    ...
    def onAnimateEndEvent(self, _):
        ...
        # Update the snake
        self.mesh_snake.vertices = self.root.snake.visual.body.getObject('OglBody').position.value
